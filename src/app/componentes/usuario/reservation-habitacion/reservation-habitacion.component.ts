...
export class ReservationHabitacionesComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private ReservationService: ReservationHabitacionService) {}
  ...
  reservar(): void {
    ...
    this.ReservationService.reservarHabitacion(
      this.habitacionSeleccionada.id, fechaInicioFormatted, fechaFinFormatted
    ).subscribe({
      next: (response) => {
